...
%
% BayFish pipeline
% PIPELINE: Applying algorithm to experimental data.
%
...
% (7) Define best sampling method.
% (8) Explore other "models" & calculate/compare evidence.
%
%   See also DATA_X.m
%   See also DATA_Xm.m
...
clear
myGene = 'Fos';
myGeneModel = 'Fos(2S,300)';

%% (1) Define data:
% (1.1) Import data using DATA_X.m. Experimental data should be in an excel file (e.g. DataMerged_Fos.xlsx) 
% where each time point measure is a sheet (e.g. 't15'), and the first 
% line has the "name" of the experiment (e.g. 'KCl (5 min) + CM (10 min)'), 
% the second line the name of the columns ('TS1', 'TS2', 'mRNA', and, 
% optionally,  'Exp.' with the experiment reference), and then the list of 
% individual cell measurements (e.g. [6.55,4.76,46,'II']).
...
% (1.3) Save file:
    ...
    clear x
% CLEAN
    clear Exp NtZ ExI N maxM myGene

%% (2) Define mathematical model:
% (2.1) If needed, create instructions for the model's transition matrix
% given the chosen limit for mRNA molecule number.
% (2.2) Calculate the transition matrixes given the model and its kinetic 
% parameters.
...
% (2.2) Calculate transition matrixes:
    ...
    clear pS i1
% CLEAN
    clear N maxM Par ParS

%% (3) Calculate probability distributions
% (3.1) Calculate the probability distributions for the chosen time points 
% the transition matrixes.
% (3.2) Transform the probability distributions vectors to a matrix form
% where M(i,j) is the probability of having the promoters state (i) and 
% exactly (j-1) mRNA molecules.
...
% (3.1) Calculate probability distributions vectors:
    ...
    clear t tt
% (2.4) Transform probability distributions vectors to matrixes:
    for t = 1:length(myT)
...
% (4.1) Calculate the log-likelihood of observing the data x given the
...
mrw.I = 100;         % Number of iterations
mrw.s = 1;              % Random number generator seed
%%% mrw.([U/S]).(par) = [sigma,min,max] %%%
mrw.U.kOFF = [1e-4,	1e-5, 0.1];
mrw.U.mu0  = [1e-4,	1e-5, 0.1];
mrw.U.mu   = [0.01,	1e-3, 10];
mrw.S.kON  = [1e-4,	1e-6, 0.01;...
              1e-4,	1e-4, 1];
%%% OUTPUT %%%
...
% Metropolis Random Walk algorithm parameters:
mrw.I = 10;       % Number of iterations
mrw.s = 1;         % Random number generator seed
%%% mrw.([U/S]).(par) = [sigma,min,max] %%%
mrw.U.kOFF = [1e-5,	1e-5, 0.1];
mrw.U.mu0  = [1e-5,	1e-5, 0.1];
mrw.U.mu   = [0.001,1e-3, 10];
